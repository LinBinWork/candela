...
*/
		,fkReady : function(f){
			...
			if (!!window._CriviaDymanicFields
			&& !!window._CriviaDymanicFields.length) {
				f(); return;
			}
...
/**
 *设置按钮对象隐藏 -field8467_0_browserbtn
 */
function setHideBut(v){
	if (isNull(v)) {
		return false;
	}
	getID(v).siblings('button:first').hide();
};
 ...
 *设置按钮对象显示
 */
function setShowBut(v){
	if (isNull(v)) {
		return false;
	}
	getID(v).siblings('button:first').show();
};
...
function mfs(k,n,f){
	...
	if ("string" == typeof jQuery("#"+k)
		...
		jQuery("#"+k).val(commafy(_C.n(v,0).toFixed(f)));
		return;
	}
	_C.v(k,_C.n(v,0).toFixed(f));
	var nv = _C.v(k);
	if (!nv || nv.indexOf(",")>-1){
		return;
	}
	if ("function"== typeof window.changeToThousands2){
		changeToThousands2(k,f);
		var nv = _C.v(k);
		if (!nv || nv.indexOf(",")>-1){
			return;
		}
	...
	if ("function"== typeof window.commafy){
		_C.v(k,commafy(_C.n(v,0).toFixed(f)));
	}
};


//Money Format Show (Read Only).
function mfsro(k,v,f){
	mfs(k,v,f);
